...
public class ControladorMessage {
    ...
    @GetMapping("/{id}")
    public Optional<ModelMessage> getMessage(@PathVariable("id") int messageId) {
        return servico.getMessage(messageId);
    }

    @PostMapping("/save")
    @ResponseStatus(HttpStatus.CREATED)
    public ModelMessage save(@RequestBody ModelMessage message) {
        return servico.save(message);
    }
    @PutMapping("/update")
    @ResponseStatus(HttpStatus.CREATED)
    public ModelMessage update(@RequestBody ModelMessage message) {
